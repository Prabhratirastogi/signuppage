...
def signup(request):
    if request.method == "POST":
        ...
        # if User.objects.filter(docid=docid):
        #     messages.error(request, "Doctor_Id already exist! Please try some other Doctor_Id.")
        #     return redirect('home')
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request, "doctor/logout.html",{"fname":fname})
        else:
